...
function Contacts(props) {
    ...
    return (
        ...
        : ( !props.darkVisible ? styles.hidden : stylesDark.hidden)}>
             ...
             <h1  className={!props.darkVisible ? styles.secondName : stylesDark.secondName}>{t('Name.Name2')}</h1>
             <h2  className={!props.darkVisible ? styles.phrase : stylesDark.phrase}>{PhrasesObj[Math.floor(Math.random() * PhrasesObj.length)]}<span className={styles.blink}>&#9612;</span></h2>
             </div><img src={!props.darkVisible ? arrows : arrows2} 
            className={styles.arrows} onClick={()=>handelPhrase(PhrasesObj)}/>
        
            <h3 className={!props.darkVisible ? styles.text_h3 : stylesDark.text_h3}><img src={!props.darkVisible ? download : download2} 
            className={styles.download}/><a href={resume} download="" className={!props.darkVisible ? styles.resume : stylesDark.resume}>{t('Contacts.resume')}</a></h3>
            <h3 className={!props.darkVisible ? styles.text_h3 : stylesDark.text_h3}><a href="https://github.com/kiraanikeev" className={!props.darkVisible ? styles.resume : stylesDark.resume}><img src={!props.darkVisible ? git : git2} 
            className={styles.git}/>{t('Contacts.git')}</a></h3>
            <h3 className={!props.darkVisible ? styles.textH3 : stylesDark.textH3}><img src={!props.darkVisible ? point : point2} 
